...
// LGT: optional setting to unlock wallet for block minting only;
...
// LGT: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // LGT: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // LGT: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // LGT: not avatar mode
                    {
...
// LGT: create coin stake transaction
...
// LGT: check 'spent' consistency between wallet and txindex
// LGT: fix wallet spent state according to txindex
...
// LGT: disable transaction (only for coinstake)
