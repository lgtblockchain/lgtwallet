...
// Copyright (c) 2011-2015 The LGT developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 27);
static CBigNum bnInitialHashTarget(~uint256(0) >> 28);
unsigned int nStakeMinAge = STAKE_MIN_AGE;
int nCoinbaseMaturity = COINBASE_MATURITY_LGT;
...
const string strMessageMagic = "LGT Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // LGT: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // LGT: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // LGT: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// LGT: find block wanted by given orphan block
...
//#1
// LGT: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// LGT: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // LGT: target change every block
    // LGT: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // LGT: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // LGT: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // LGT: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // LGT: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // LGT: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // LGT: track money supply and mint amount info
    ...
    // LGT: fees are not collected by miners as in bitcoin
    // LGT: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// LGT: total coin age spent in transaction, in the unit of coin-days.
...
// LGT: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // LGT: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // LGT: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // LGT: record proof-of-stake hash value
...
bool CBlock::CheckBlock() const
{
    ...
    // LGT: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // LGT: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // LGT: check that the block satisfies synchronized checkpoint
    ...
    // LGT: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // LGT: verify hash target and signature of coinstake tx
    ...
    // LGT: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // LGT: getblocks may not obtain the ancestor block rejected
    ...
    // LGT: if responsible for sync-checkpoint send it
...
// LGT: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "LGT", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "LGT", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //#1
        // Genesis Block:
        // block.nTime = 1502033462
        //block.nNonce = 3533667
        //block.GetHash = 00000014ec1393ee5b37c8e2a7f05a6200c40e13ec015528ff0846c74a7ab115
        //CBlock(hash=00000014ec1393ee5b37, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=11220e2fa1, nTime=1502033462, nBits=1d1fffff, nNonce=3533667, vtx=1, vchBlockSig=)
        //  Coinbase(hash=11220e2fa1, nTime=1502033407, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //    CTxIn(COutPoint(0000000000, -1), coinbase 04ffff001d020f27244b4c43206973206d792070726f6a6563742066696e69736820696e20323031372f382f37)
        //    CTxOut(empty)
        //  vMerkleTree: 11220e2fa1


        // Genesis block
        const char* pszTimestamp = "LGT is my project finish in 2017/9/8";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1504873800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 199552622;
        ...
        assert(block.hashMerkleRoot == uint256("0x341eaf4a899df7209fc3c8f0adcb7f4e4fc6f28cabc7fb21156cd934d9c63fbb"));
        ...
        // LGT: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // LGT: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // LGT: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // LGT: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // LGT: should not enter safe mode for longer invalid chain
    // LGT: if sync-checkpoint is too old do not enter safe mode
    ...
    // LGT: if detected invalid checkpoint enter safe mode
    ...
    // LGT: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // LGT: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // LGT: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // LGT: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // LGT: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // LGT: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // LGT: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // LGT: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
